...
class Attention(nn.Module):
    ...
    def forward(self, x):
        assert(len(x.shape)==3), "输入向量的shape必须为3"
        Q = self.query(x)
        K = self.key(x)
        V = self.value(x)
        
        W = F.softmax(torch.bmm(Q, K.transpose(1,2)) / torch.sqrt(self.embedding_size),dim=2)
...
class Net(nn.Module):
    def __init__(self, batch_size=64):
        ...
        self.embed_size = 28
        self.attention1 = Attention(self.embed_size)
        # self.attention2 = Attention(self.embed_size)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        writer.add_scalar("loss/epoch_{}".format(epoch), loss.item(),batch_idx)
