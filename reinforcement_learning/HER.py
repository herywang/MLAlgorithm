# Hindsight Experience Replay 论文地址: https://arxiv.org/abs/1707.01495v1
...
class DQNAgent():
    ...
    def _set_model(self):  # set value network
        ...
        # 更新方式: target_net参数的 5% + eval_net参数的95%
    ...
    def choose_action(self, state, goal):
        ...
        # use tf.argmax is much slower, so use np
        return np.argmax(act_values[0])

    def remember(self, ep_experience):
        self.memory += ep_experience.memory
        if len(self.memory) > self.buffer_size:
            # empty the first memories
    ...
    def replay(self, optimization_steps):
        ...
        for _ in range(optimization_steps):
            ...
            loss, _ = self.sess.run([self.loss, self.train_op], {self.tfs: ss, self.tfg: gs, self.tfa: acs, self.tfr: rs, self.tfs_: nss, self.tfd: ds})
            losses += loss
...
if __name__ == '__main__':
    ...
    ep_experience = EpisodeExperience()
    ep_experience_her = EpisodeExperience()
            ...
            if use_her:  # The strategy can be changed here
                ...
                for t in range(len(ep_experience.memory)):
                    for k in range(K):
                        ...
                        # next_state of future
                        goal = ep_experience.memory[future][3]
