...
class AgentDSR(object):
    def __init__(self, env, n_steps=3000, output_graph=False):
        ...
        self.start_learning_after_n_step = 200
        ...
        self.epsilon_decrement = (self.epsilon - self.epsilon_min) / 1000.
        self.reward_his = []
    ...
    def __build_model(self):
        ...
        self.target_mu_p = tf.placeholder(tf.float32, [None, self.fi_size * self.n_actions], name='target_mu')
        ...
        with tf.variable_scope("mu"):
            mu1 = tf.layers.dense(self.fi_eval_p, 64, tf.nn.relu6, kernel_initializer=w_initializer, name='mu1')
            mu2 = tf.layers.dense(mu1, 64, tf.nn.relu6, kernel_initializer=w_initializer, name='mu2')
            self.mu = tf.layers.dense(mu2, self.fi_size * self.n_actions, tf.nn.relu6, kernel_initializer=w_initializer, name='mu3')
        ...
        with tf.variable_scope('mu_loss'):
            self.loss3 = tf.reduce_mean(tf.squared_difference(self.mu, self.target_mu_p, name='loss3'))

        self.loss = self.loss1 + self.loss2 + self.loss3

        with tf.variable_scope('train'):
            self.train_op1 = tf.train.AdamOptimizer(0.001, 0.9).minimize(self.loss1)
            self.train_op2 = tf.train.AdamOptimizer(0.001, 0.9).minimize(self.loss2)
            self.train_op3 = tf.train.AdamOptimizer(0.001, 0.9).minimize(self.loss3)
    ...
    def learn(self, step):
        ...
        mu_ = self.sess.run(self.mu, feed_dict={self.fi_eval_p: fi_})
        mu_ = np.reshape(mu_, (self.batch_size, self.n_actions, self.fi_size))
        target_mu = np.zeros((self.batch_size, self.n_actions, self.fi_size))
        for i in range(self.batch_size):
            qs_ = np.matmul(mu_[i], w)
            a_index_ = np.argmax(qs_)
            target_mu[i, actions[i], :] = fi[i] + self.gamma * mu_[i, a_index_, :]
        target_mu = np.reshape(target_mu, (self.batch_size, -1))
        loss3, _ = self.sess.run([self.loss3, self.train_op3], feed_dict={self.fi_eval_p: fi, self.target_mu_p: target_mu})

        if self.output_graph:
            [s_loss1, s_loss2, s_loss3, s_loss] = self.sess.run(self.summary, feed_dict={self.loss1_p: loss1, self.loss2_p: loss2,
                                                                                         self.loss3_p: loss3, self.loss_p: loss1 + loss2 + loss3})
        ...
        while step < self.n_steps:
            ...
            while not done:
                ...
                if step > self.start_learning_after_n_step:
                    ...
                    print('Step: %i, Episode: %i, Action:%i, Reward:%.2f, Epsilon: %.5f, Loss1:%.8f, Loss2:%.8f, Loss3:%.8f'
    ...
    def choice_action(self, observation, test=False):
        ...
        if np.random.uniform() > self.epsilon:
            ...
            mus_eval = self.sess.run(self.mu, feed_dict={self.fi_eval_p: encoded})
            mus_eval = np.reshape(mus_eval, (self.n_actions, self.fi_size))
            mus = tf.constant(mus_eval)
